...
DEFAULT_OUTPUT_PATH = Path(__file__).resolve().with_name("urbanflux_borough_assignment_training.jsonl")
...
def csv_headers(source_row: object) -> list[str]:
    if isinstance(source_row, dict):
        return [str(key) for key in source_row.keys()]
    if isinstance(source_row, list):
        return [f"column_{index + 1}" for index in range(len(source_row))]
    return []


def metadata_description(source: dict[str, Any]) -> str:
    parts = [
        ("Dataset title", source.get("dataset_title")),
        ("Dataset description", source.get("dataset_description")),
        ("Resource title", source.get("resource_title")),
        ("Resource description", source.get("resource_description")),
        ("Organisation", source.get("organisation_name")),
        ("Dataset URL", source.get("dataset_url")),
        ("CSV URL", source.get("csv_url")),
    ]
...
def output_record(row: dict[str, Any]) -> dict[str, Any]:
    return {
        "messages": [
            ...
            {"role": "assistant", "content": str(row["borough_name"])},
        ]
    }
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prepare no-LLM UrbanFlux borough-assignment fine-tune JSONL data."
    )
